...
<head>
    ...
    <title>@yield('title', 'Dashboard Petugas - Perpustakaan Digital')</title>
            ...
            <div class="h-20 flex items-center px-6 border-b border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-slate-400 font-medium">Sistem Digital</div>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-3 space-y-1">
                ...
                <div class="mb-6">
                    ...
                    <a href="{{ route('petugas.dashboard') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.dashboard') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-th-large w-5 text-center {{ request()->routeIs('petugas.dashboard') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                        <span class="font-medium">Dashboard</span>
                    </a>

                    <a href="{{ route('petugas.anggota.index') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.anggota*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-users w-5 text-center {{ request()->routeIs('petugas.anggota*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                        <span class="font-medium">Data Anggota</span>
                    </a>

                    <a href="{{ route('petugas.buku.index') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.buku*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-book w-5 text-center {{ request()->routeIs('petugas.buku*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                ...
                <!-- Group: Transaksi -->
                <div class="mb-6">
                    <div class="px-3 mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Transaksi</div>
                    
                    <a href="{{ route('petugas.peminjaman.index') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.peminjaman*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-hand-holding-book w-5 text-center {{ request()->routeIs('petugas.peminjaman*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                        <span class="font-medium">Peminjaman</span>
                    </a>

                    <a href="{{ route('petugas.pengembalian.index') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.pengembalian*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-undo w-5 text-center {{ request()->routeIs('petugas.pengembalian*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                        <span class="font-medium">Pengembalian</span>
                    </a>

                    <a href="{{ route('petugas.denda.index') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.denda*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-money-bill-wave w-5 text-center {{ request()->routeIs('petugas.denda*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                ...
                <!-- Group: Lainnya -->
                <div>
                    <div class="px-3 mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Lainnya</div>
                    
                    <a href="{{ route('petugas.laporan.index') }}" 
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('petugas.laporan*') ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} transition group">
                        <i class="fas fa-chart-pie w-5 text-center {{ request()->routeIs('petugas.laporan*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
            ...
            <div class="p-4 border-t border-slate-800 bg-[#0f172a]">
                <div class="flex items-center gap-3 p-2 rounded-xl bg-slate-800/50 border border-slate-700/50">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-xs text-slate-400 truncate">Petugas Perpustakaan</div>
                    </div>
                ...
                <div class="flex items-center gap-6">
                    ...
                    <form action="{{ route('petugas.search') }}" method="GET" class="hidden md:block relative">
                        <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari buku, anggota, atau transaksi..." 
                               class="w-64 pl-10 pr-4 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" 
                            ...
                            <div class="px-4 py-3 border-b border-slate-700 bg-slate-800/50 flex justify-between items-center">
                                ...
                                <span class="text-xs text-indigo-400 font-semibold">{{ $totalNotif }} Baru</span>
                            </div>
                            ...
                            <div class="px-4 py-2 bg-slate-800/50 border-t border-slate-700 text-center">
                                <a href="{{ route('petugas.peminjaman.index') }}" class="text-xs text-indigo-400 hover:text-indigo-300 font-medium">Lihat Semua Aktivitas &rarr;</a>
                            </div>
                        </div>
                    </div>

                    <!-- 3. Date Time (Tetap) -->
                    <div class="hidden lg:block text-right border-l border-slate-700 pl-6">
